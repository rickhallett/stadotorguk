...
export function SignUpForm({
  ...
  const onSubmit = useCallback(async (formValues: SignUpFormValues) => {
    ...
    try {
      ...
      const errorMessage = 'There was an error submitting your information. Please try again later.';
      setSubmissionError(errorMessage);
  ...
  }, [apiEndpoint, confirmationDuration, onSubmitSuccess, onSubmitError, resetForm, setFieldError]);

  // Handle input changes
  const handleInputChange = useCallback((field: keyof SignUpFormValues) => 
  ...
  const handleInputBlur = useCallback((field: keyof SignUpFormValues) => 
    ...
    <div className={`signup-form-container ${className}`} {...props}>
      <div className="action-form">
        {showConfirmation ? (
          ...
          <div className="form-group">
            <label htmlFor="signup-message">YOUR STORY (OPTIONAL)</label>
            <textarea
              id="signup-message"
              name="message"
            ...
            />
            <small id="character-count" className="character-counter">
              Characters remaining: <span className={remainingChars < 100 ? 'warning' : ''}>
                {remainingChars}
              </span>
            </small>
